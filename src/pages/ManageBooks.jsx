    ...
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <div className="w-full px-4 py-3 bg-white border border-emerald-200 rounded-xl focus-within:ring-2 focus-within:ring-emerald-500 focus-within:border-emerald-500 text-emerald-900 flex items-center justify-between">
          <span className={selectedOption ? 'text-emerald-900' : 'text-gray-400'}>
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-emerald-200 rounded-xl shadow-lg max-h-80 overflow-hidden">
              ...
              filteredOptions.map((option) => (
                ...
                >
                  {renderOption ? renderOption(option) : (
                    <div className="font-medium text-emerald-900">{getOptionLabel(option)}</div>
                  )}
                </div>
              ))
            ) : (
...
// Updated Book Modal Component with Searchable Dropdowns
const BookModal = ({ 
  isOpen, 
  onClose, 
  title, 
  ...
  return (
    <div className="fixed inset-0 bg-emerald-900 bg-opacity-50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto mt-15">
        <div className="sticky top-0 z-10 bg-gradient-to-r from-emerald-500 to-green-600 text-white p-6 rounded-t-2xl">
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-2xl font-bold mb-1">{title}</h2>
        ...
        <div className="p-6">
          <form onSubmit={onSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Searchable Bookstore Select */}
              <SearchableSelect
                options={bookstores}
                value={formData.bookstore_id}
                onChange={(value) => onChange('bookstore_id', value)}
                placeholder="-- Select a Bookstore --"
                label="Select Bookstore"
                required={true}
                getOptionValue={(option) => option.id}
                getOptionLabel={(option) => `${option.name} - ${option.location || 'No location'}`}
                renderOption={(option) => (
                  <div>
                    <div className="font-medium text-emerald-900">{option.name}</div>
                    {option.location && (
                      <div className="text-sm text-emerald-600 mt-0.5">{option.location}</div>
                    )}
                  </div>
                )}
                noOptionsMessage="No bookstores found"
              />
              ...
              <SearchableSelect
                ...
                placeholder={!formData.bookstore_id ? "Select a bookstore first" : "-- Select an Author --"}
                label="Select Author"
                required={true}
                disabled={!formData.bookstore_id}
                ...
                renderOption={(option) => (
                  <div className="flex items-center space-x-3">
                    <div className="w-8 h-8 bg-emerald-100 rounded-full flex items-center justify-center overflow-hidden">
                      {renderAuthorAvatar(option.avatar, option.name)}
                    </div>
                    <div>
                      <div className="font-medium text-emerald-900">{option.name}</div>
                      {option.genre && (
                        <div className="text-xs text-emerald-600">{option.genre}</div>
                      )}
                    </div>
                  </div>
                )}
                noOptionsMessage={!formData.bookstore_id ? "Select a bookstore first" : "No authors found. Add authors first."}
              />
            </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-emerald-700 mb-2">Book Title *</label>
                  <input type="text" name="title" value={formData.title} onChange={(e) => onChange('title', e.target.value)} required className="w-full px-4 py-3 border border-emerald-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" placeholder="Enter book title" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-emerald-700 mb-2">Price ($)</label>
                  <input type="number" name="price" value={formData.price} onChange={(e) => onChange('price', e.target.value)} step="0.01" className="w-full px-4 py-3 border border-emerald-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" placeholder="e.g., 19.99" min="0" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-emerald-700 mb-2">Genre</label>
                  <input type="text" name="genre" value={formData.genre} onChange={(e) => onChange('genre', e.target.value)} className="w-full px-4 py-3 border border-emerald-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" placeholder="e.g., Fiction, Mystery, Science" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-emerald-700 mb-2">Published Date</label>
                  <input type="date" name="published_date" value={formData.published_date} onChange={(e) => onChange('published_date', e.target.value)} className="w-full px-4 py-3 border border-emerald-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-emerald-700 mb-2">ISBN</label>
                <input type="text" name="isbn" value={formData.isbn} onChange={(e) => onChange('isbn', e.target.value)} className="w-full px-4 py-3 border border-emerald-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" placeholder="e.g., 978-3-16-148410-0" />
              </div>
            </div>

            <div>
              <label className="block text-sm font-medium text-emerald-700 mb-2">Description</label>
              <textarea name="description" value={formData.description} onChange={(e) => onChange('description', e.target.value)} rows="4" className="w-full px-4 py-3 border border-emerald-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" placeholder="Describe the book..." />
            </div>
