...
const Login = () => {
  ...
  // API helper function with credentials for session cookies
  const apiCall = async (endpoint, options = {}) => {
    const url = `${config.apiBaseUrl}${endpoint}`;
    const defaultOptions = {
      headers: {
      ...
      credentials: 'include', // Important: This sends/receives cookies for session
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Call your existing session-based login API (same as bookstore login)
      const response = await apiCall('/auth/login', {
        method: 'POST',
        body: JSON.stringify({ email, password })
      });
      ...
      if (response.success) {
        ...
        // No need to store token - session is handled by cookie
        // Just store user info in localStorage for UI state if needed
        if (response.data.user) {
