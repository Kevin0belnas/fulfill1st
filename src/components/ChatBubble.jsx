...
const ChatBubble = React.forwardRef((props, ref) => {
  ...
  // Fetch agent full profile (with image/avatar)
  const fetchAgentFullProfile = async (agentId) => {
    try {
      // Try to fetch from public profile endpoint
      const response = await apiCall(`/agent/profile/public/${agentId}`);
      if (response.success && response.data) {
      ...
      console.log('Could not fetch agent profile, using default');
      return null;
    }
  };
  ...
  const fetchAgentInfo = async (agentId) => {
    try {
      ...
      if (response.success) {
        ...
        // Fetch agent profile (profile_image, avatar_type)
        const profileData = await fetchAgentFullProfile(agentId);
        if (profileData) {
  ...
  // Get agent avatar display for header and messages
  const getAgentAvatar = () => {
    if (!agentProfile) {
      // Return null to show default
      return null;
    }
    
    if (agentProfile.profile_image) {
      return (
        <img 
          src={agentProfile.profile_image} 
    ...
    if (agentProfile.avatar_type === 'male') {
      return <FaMars className="text-2xl text-white" />;
    }
    
    if (agentProfile.avatar_type === 'female') {
      return <FaVenus className="text-2xl text-white" />;
    }
    
    return null;
  };

  // Get agent avatar for message display (smaller)
  const getAgentMessageAvatar = () => {
    if (!agentProfile) {
      return null;
    }
    ...
    if (agentProfile.avatar_type === 'male') {
      return <FaMars className="text-sm text-white" />;
    }
    
    if (agentProfile.avatar_type === 'female') {
      return <FaVenus className="text-sm text-white" />;
    }
  ...
  const saveMessage = async (messageText, isAgent = false, agentId = null, conversationId = null) => {
    if (!email && !isAgent) {
      console.error('❌ No email set, cannot save visitor message');
      return null;
    }
          ...
          const newMessages = response.data.filter(msg => {
            ...
            const existsInCurrent = currentMessages.some(existing => {
              ...
              if (existing.text === msg.message && existing.from === (msg.is_agent ? 'agent' : 'user')) {
                const existingTime = new Date(getMessageTimestamp(existing)).getTime();
                const msgTime = new Date(msg.created_at || msg.timestamp).getTime();
                if (Math.abs(existingTime - msgTime) < 3000) return true;
              }
              return false;
            });
          ...
          if (newMessages.length > 0) {
            newMessages.forEach(msg => {
              ...
              setMessages(prev => {
                ...
                const withoutTemp = prev.filter(m => !(m.isTemporary && m.text === formattedMsg.text));
                const updatedMessages = [...withoutTemp, formattedMsg];
                return sortMessagesByTime(updatedMessages);
              });
              
              if (msg.is_agent && msg.agent_id && !agent) {
                fetchAgentInfo(msg.agent_id);
              }
            });
            scrollToBottom();
  ...
  const fetchMessages = useCallback(async (conversationId) => {
    try {
      ...
      if (response.success && response.data) {
        ...
        const agentMessage = response.data.find(msg => msg.is_agent && msg.agent_id);
        if (agentMessage && agentMessage.agent_id) {
          await fetchAgentInfo(agentMessage.agent_id);
          setWaitingForAgent(false);
          setIsTyping(false);
        }
  ...
  const assignAgentToVisitor = async (visitorEmail, topic, conversationId) => {
    try {
      ...
      if (response.success && response.data && response.data.agent) {
        setAgent(response.data.agent);
        // Fetch agent profile after assignment
        const profileData = await fetchAgentFullProfile(response.data.agent.id);
        if (profileData) {
          setAgentProfile(profileData);
        }
        return response.data.agent;
  ...
  // Handle topic selection
  const handleTopicSelect = async (topic) => {
    if (isSendingRef.current || topicSavedRef.current) return;
  ...
  const handleEmailSubmit = async () => {
    ...
    try {
      ...
      const assignedAgent = await assignAgentToVisitor(email, topic, newConversationId);
      if (assignedAgent) {
        startPolling(newConversationId);
      }
      pendingTopicRef.current = null;
  ...
  const handleSend = async () => {
    ...
    const savedMessage = await saveMessage(messageText, false, null, currentConversationId.current);
  ...
  const headerAvatar = getAgentAvatar();
        ...
        <div className="w-96 h-[600px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-200">
          ...
          <div className="flex-1 p-0 overflow-y-auto flex flex-col bg-gradient-to-br from-gray-50 to-blue-50 scrollbar-hide">
            ...
            {!selectedTopic && !requestEmail && showWelcome ? (
              ...
              <div className="flex-1 p-5 flex flex-col gap-3">
                {messages.map((msg, index) => {
                  const isAgentMessage = msg.from === 'agent';
                  const messageAvatar = isAgentMessage ? getAgentMessageAvatar() : null;
                  ...
                  return (
                    ...
                    >
                      {isAgentMessage && agent && (
                        <div className="flex items-center mb-1 pl-2">
                          <div className="w-5 h-5 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center text-white text-xs overflow-hidden mr-2">
                            {messageAvatar || agent.name?.charAt(0).toUpperCase()}
                          </div>
                          <span className="font-semibold text-xs text-gray-600">{agent.name}</span>
                        </div>
                      )}
